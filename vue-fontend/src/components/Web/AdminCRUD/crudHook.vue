...
<v-card>
    <v-card-title>
        <v-btn @click="hookDialogSwitch(true)" color="success">Add</v-btn>
        <v-spacer></v-spacer>
    ...
    <v-data-table :headers="headers" :items="hookList" :search="search">
        <template v-slot:items="props" text-xs-center>
            ...
            <td>
                <v-btn @click="getHookOnce( props.item.id )" small color="primary">Edit</v-btn><br/>
                <v-btn @click="deleteHook( props.item.id )" small color="error">Delete</v-btn>
            </td>
        </template>
