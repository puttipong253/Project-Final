...
<v-card>
    <v-card-title>
        <v-btn @click="lineDialogSwitch(true)" color="success" fab><v-icon>add</v-icon></v-btn>
        <v-spacer></v-spacer>
    ...
    <v-data-table :headers="headers" :items="lineList" :search="search">
        <template v-slot:items="props" text-xs-center>
            ...
            <td>
                <v-btn @click="getLineOnce( props.item.id )" fab small color="primary"><v-icon>create</v-icon></v-btn>
                <v-btn @click="deleteLine( props.item.id )" fab small color="error"><v-icon>delete_outline</v-icon></v-btn>
            </td>
        </template>
